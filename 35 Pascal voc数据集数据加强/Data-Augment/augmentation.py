...
if __name__ == "__main__":
    ...
    AUG_XML_DIR = "xml"  # 存储增强后的XML文件夹路径
    try:
    ...
    seq = iaa.Sequential([
        ...
        iaa.Multiply((0.8, 1.0)),  # change brightness, doesn't affect BBs
        ...
        for name in files:
            ...
            for epoch in range(AUGLOOP):
                ...
                path = os.path.join(AUG_IMG_DIR,
                                    str("%06d" % (len(files) + int(name[:-4]) + epoch * AUGNUMBER)) + '.jpg')
                ...
                change_xml_list_annotation(XML_DIR, name[:-4], new_bndbox_list, AUG_XML_DIR,
                                           len(files) + int(name[:-4]) + epoch * AUGNUMBER)
                print(str("%06d" % (len(files) + int(name[:-4]) + epoch * AUGNUMBER)) + '.jpg')
                # print('len(files) = ',str(len(files)) + 'int(name[:-4]) = '+ str(int(name[:-4])) +'epoch * 250 = '+ str(epoch * 250))
                new_bndbox_list = []
