...
def xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data):
    ...
    if min_iCountD == 1:    #边界线切到多个目标框
        #都变成<0.5的数字，取最小
        #print('iCountD = ', iCountD)
        #print('overlapRate_All = ', overlapRate_All)
...
def yt_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data):
    ...
    if flag == 1 :
        ...
        #print('我执行在yt minIndex里面------------------',minIndex)
        return bStep * minIndex
...
def xr_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data):
    ...
    if flag == 1 :
        ...
        #print('我执行在xr minIndex里面------------------',minIndex)
        return bStep * minIndex
...
def yb_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data):
    ...
    if flag == 1 :
        ...
        #print('我执行在yb minIndex里面------------------',minIndex)
        return bStep * minIndex
    ...
    for i in range(len(data)):
        ...
        if ratio == 1:
            ...
            if x2 + w2 > slide_x1 + x_bSteps:
                #w2 = x_bSteps - (slide_x1 + x_bSteps - x1)
                w2 = slide_x1 + x_bSteps - x2
            if y2 + h2 > slide_y1 + y_bSteps:
                #h2 = y_bSteps - (slide_y1 + y_bSteps - y2)
        ...
        while (slide_y1 + bSteps < img.shape[0]):
            ...
            while (slide_x1 + bSteps < img.shape[1]):
                ...
                if(max(overlapRate) > 1 / 2 and max(overlapRate) < 1):
                    ...
                    '''
                    cropped = img[slide_y1 : slide_y1 + y_bSteps, slide_x1 : slide_x1 + x_bSteps]
                    imgPath = '/home/huangfu/testX完整/'+str(index)+'.jpg'
                    print(str(index)+'.jsg')
                    index += 1
                    cv2.imwrite(imgPath,cropped)

                    cv2.rectangle(img,(slide_x1, slide_y1), (slide_x1 + x_bSteps, slide_y1 + y_bSteps),(106,90,205),3)
                    '''

                if(max(overlapRate) == 1):
                    #print('我是overlapRate',overlapRate)
                    #有完整目标里面考虑残缺目标
                    while max(overlapRate) == 1:
                        overlapRate.remove(1)
                    ...
                    #print('大的 = ',lNumber,'小的 = ',sNumber)

                    #分类讨论
                    if max(overlapRate) == 0 :
                        '''
                        cropped = img[slide_y1 : slide_y1 + y_bSteps, slide_x1 : slide_x1 + x_bSteps]
                        imgPath = '/home/huangfu/testX完整/'+'直接'+str(index)+'.jpg'
                        print('直接'+str(index)+'.jsg')
                        index += 1
                        cv2.imwrite(imgPath,cropped)
                        '''

                    elif lNumber >= sNumber:
                        #扩大四条边的范围
                        ...
                        '''
                        cropped = img[slide_y1 : slide_y1 + y_bSteps, slide_x1 : slide_x1 + x_bSteps]
                        imgPath = '/home/huangfu/testX完整/'+'扩大'+str(index)+'.jpg'
                        print('扩大'+str(index)+'.jsg')
                        index += 1
                        cv2.imwrite(imgPath,cropped)
                        

                        cv2.rectangle(img,(slide_x1, slide_y1), (slide_x1 + x_bSteps, slide_y1 + y_bSteps),(28,28,28),3)
                        '''
                    ...
                    #在(0,0.5)的数量比较小
                        ...
                        '''
                        cropped = img[slide_y1 : slide_y1 + y_bSteps, slide_x1 : slide_x1 + x_bSteps]
                        imgPath = '/home/huangfu/testX完整/'+'缩小'+str(index)+'.jpg'
                        print('缩小'+str(index)+'.jsg')
                        index += 1
                        cv2.imwrite(imgPath,cropped)


                        cv2.rectangle(img,(slide_x1, slide_y1), (slide_x1 + x_bSteps, slide_y1 + y_bSteps),(0,0,255),3)
                        '''
