...
def area_overlab(x1, y1, w1, h1, data):
    ...
    for j in range(len(data)):
        ...
        y2 = data.values[j][1]
        # 修改
        w2 = data.values[j][2] - data.values[j][0]
        h2 = data.values[j][3] - data.values[j][1]
...
#左边的线
def xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, expand_i):
    ...
    for step in range(0,allStep,bStep):
        for k in range(len(data)):
            ...
            sq = [data.values[k][0], data.values[k][3], data.values[k][2], data.values[k][1]]
            ...
            if D == 1:
                ...
                if expand_i == 1:
                    overlapRate = area_overlab_double(slide_x1-step, slide_y1, x_bSteps+step, y_bSteps, data.values[k][0], data.values[k][1], data.values[k][2]-data.values[k][0], data.values[k][3]-data.values[k][1])
                    overlapRate_double.append(overlapRate)
                elif expand_i == 2:
                    overlapRate = area_overlab_double(slide_x1, slide_y1-step, x_bSteps, y_bSteps+step, data.values[k][0], data.values[k][1], data.values[k][2]-data.values[k][0], data.values[k][3]-data.values[k][1])
                    overlapRate_double.append(overlapRate)
                elif expand_i == 3:
                    overlapRate = area_overlab_double(slide_x1, slide_y1, x_bSteps+step, y_bSteps, data.values[k][0], data.values[k][1], data.values[k][2]-data.values[k][0], data.values[k][3]-data.values[k][1])
                    overlapRate_double.append(overlapRate)
                elif expand_i == 4:
                    overlapRate = area_overlab_double(slide_x1, slide_y1, x_bSteps, y_bSteps+step, data.values[k][0], data.values[k][1], data.values[k][2]-data.values[k][0], data.values[k][3]-data.values[k][1])
                    overlapRate_double.append(overlapRate)
                elif expand_i == 5:
                    overlapRate = area_overlab_double(slide_x1+step, slide_y1, x_bSteps-step, y_bSteps, data.values[k][0], data.values[k][1], data.values[k][2]-data.values[k][0], data.values[k][3]-data.values[k][1])
                    overlapRate_double.append(overlapRate)
...
def writePicAndXml(slide_x1, slide_y1, x_bSteps, y_bSteps, data):
    ...
    for i in range(len(data)):
        x2 = data.values[i][0]
        y2 = data.values[i][1]
        w2 = data.values[i][2] - data.values[i][0]
        h2 = data.values[i][3] - data.values[i][1]
...
# datPath = '/home/huangfu/桌面/图片dat/dat/'
# imagePath = '/home/huangfu/桌面/图片dat/图片/'
# saveAllImagePath = '/home/huangfu/testX全部/'
# saveDefectImagePath = '/home/huangfu/testX残缺/'
# saveAllVocXmlPath = '/home/huangfu/testXVoc全部/'
# saveDefectVocXmlPath = '/home/huangfu/testXVoc残缺/'
# datPath = 'f:/ship/船dat/'
# imagePath = 'f:/ship/船tif/'
datPath = 'f:/ship/船dat/'
imagePath = 'f:/ship/船tif/'
saveAllImagePath = 'd:/2_ship/ship_pic_all/'
saveDefectImagePath = 'd:/2_ship/ship_pic_incomplete/'
saveAllVocXmlPath = 'd:/2_ship/ship_xml_all/'
saveDefectVocXmlPath = 'd:/2_ship/ship_xml_incomplete/'
                ...
                while (slide_x1 + bSteps < img.shape[1]):
                    ...
                    if(max(overlapRate) > 1 / 2 and max(overlapRate) < 1):
                        ...
                        if(slide_x1 == 0):
                            ...
                            #计算有没有在这条边重叠
                            differ_xl = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 1)
                            slide_x1 -= differ_xl
                            x_bSteps += differ_xl
                        ...
                        if(slide_y1 == 0):
                            ...
                            #计算有没有在这条边重叠
                            differ_yt = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 2)
                            slide_y1 -= differ_yt
                            y_bSteps += differ_yt

                        # 判断右边
                        #计算有没有在这条边重叠
                        differ_xr = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 3)
                        x_bSteps += differ_xr

                        # 判断下边
                        #计算有没有在这条边重叠
                        differ_yb = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 4)
                        y_bSteps += differ_yb

                        #print(differ_xl, differ_yt, differ_xr, differ_yb)

                    if(max(overlapRate) == 1):
                        #有完整目标里面考虑残缺目标
                        while max(overlapRate) == 1:
                            overlapRate.remove(1)
                        ...
                        sNumber = 0
                        lNumber = 0
                        # 统计overlapRate在(0,0.5)和(0.5,1)中出现的个数
                        for getNumber in overlapRate:
                            if(getNumber > 0 and getNumber < 0.5):
                                sNumber += 1
                            elif (getNumber > 0.5 and getNumber < 1):
                                lNumber += 1
                        ...
                        elif (lNumber+1) >= sNumber:  # 更多的希望扩大，而不是缩小
                            #扩大四条边的范围
                        ...
                        #在(0,0.5)的数量比较小
                            ...
                            # 判断左边
                            #计算有没有在这条边重叠
                            differ_XR = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 5)
                            slide_x1 += differ_XR
                            x_bSteps -= differ_XR

                            # 判断上边
                            #计算有没有在这条边重叠
                            differ_YB = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 6)
                            slide_y1 += differ_YB
                            y_bSteps -= differ_YB

                            # 判断右边
                            #计算有没有在这条边重叠
                            differ_XL = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 7)
                            x_bSteps -= differ_XL

                            # 判断下边
                            #计算有没有在这条边重叠
                            differ_YT = xl_location(slide_x1, slide_y1, x_bSteps, y_bSteps, data, 8)
                            y_bSteps -= differ_YT

                            # 缩小后判断有没有目标
                            iHas = area_overlab(slide_x1, slide_y1, x_bSteps, y_bSteps, data)
                            if(max(iHas) == 0):
                                slide_x1 += x_bSteps
                                continue


                    getBoxNums = area_overlab(slide_x1, slide_y1, x_bSteps, y_bSteps, data)
                    if len(getBoxNums) >= 4:
