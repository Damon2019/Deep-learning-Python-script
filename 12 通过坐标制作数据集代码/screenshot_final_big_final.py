...
def area_overlab_compare_dat(x1, y1, w1, h1, data):
    ...
    for j in range(len(data)):
        ...
        if(overlap_area / area2 > 0):
            overlapRate.append(overlap_area / area2)
            # 船的size 大于800 则使flag_4 = false
            if w2 > 800 or h2 > 800:
                flag_size = 1
            if gather == 1:
                flag_4 = 1

    return overlapRate, flag_size, flag_4
...
def writePicAndXml(slide_x1, slide_y1, x_bSteps, y_bSteps, data):
    ...
    ratio_x = x_bSteps / x_bSteps
    ratio_y = y_bSteps / y_bSteps
    ...
    node_size = SubElement(node_root, 'size')
    node_width = SubElement(node_size, 'width')
    node_width.text = str(x_bSteps)
    node_height = SubElement(node_size, 'height')
    node_height.text = str(y_bSteps)
    node_depth = SubElement(node_size, 'depth')
    node_depth.text = '3'
...
# datPath = 'd:/0-dataset-all/船dat/'
# imagePath = 'd:/0-dataset-all/船tif/'
saveAllImagePath = 'd:/4_ship/ship_pic_all/'
saveDefectImagePath = 'd:/4_ship/ship_pic_incomplete/'
saveAllVocXmlPath = 'd:/4_ship/ship_xml_all/'
saveDefectVocXmlPath = 'd:/4_ship/ship_xml_incomplete/'
        ...
        try:
            ...
            while (slide_y1 + bSteps < img.shape[0]):
                ...
                while (slide_x1 + bSteps < img.shape[1]):
                    ...
                    # 判断数量
                    getBoxNums, flag_size, flag_4= area_overlab_compare_dat(slide_x1, slide_y1, x_bSteps, y_bSteps, data)
                    # if len(getBoxNums) > 3 and max(getBoxNums) > 0.8 and flag_4 == true:
                    if flag_size == 1 and flag_4 ==0:
