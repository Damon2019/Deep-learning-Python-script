...
with open("C:/Users/win 10/Desktop/困难/picall.txt", 'r') as file:
    for line in file:
        line = line.strip('\n').split("_")[1].split(".")[0]
        print(line)
        ...
        try:
            ...
            while (slide_y1 + bSteps < img.shape[0]):
                ...
                while (slide_x1 + bSteps < img.shape[1]):
                    ...
                    # 统计残缺
                    # BN = []
                    # for gBN in range(len(getBoxNums)):
                    #     if getBoxNums[gBN] > 0 and getBoxNums[gBN] < 1:
                    #         BN.append(getBoxNums[gBN])
                    #
                    #
                    # print('getBoxNums',getBoxNums)
                    # if BN:
                    #     if min(BN) > 0.4 and flag_4 == 0 and max(BN) < 0.7:
                    #         writePicAndXml(slide_x1, slide_y1, x_bSteps, y_bSteps, data)
                    #         index += 1
