...
def classification(PATH,path1):
    PATH = PATH + '/'

    for dirnames, dirs, files in os.walk(PATH):
        for file in files:

            iFile = file.split(".")[0]

            iFileXml = file.split(".")[1]
            xml = PATH + iFile+ '.xml'
            xmlS = path1 + iFile + '.xml'
            if iFileXml == 'xml':
                fp = open(xml,'r')
                fp1 = open(xmlS,'w')
                for i in fp:
...
if __name__ == "__main__":
    ...
    path1=('/home/huangfu/桌面/NWPUdataset/annotation/')
    deal(path)
    dealwh(path)
    classification(path, path1)
