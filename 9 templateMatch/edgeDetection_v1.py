...
# 读取模板图片
template = cv2.imread('/home/huangfu/桌面/face/airplane.jpg')
# 转换为灰度图片
template = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
#高斯模糊
template = cv2.GaussianBlur(template,(5,5),0)
# 执行边缘检测
template = cv2.Canny(template, 20, 250)
...
# 遍历所有的图片寻找模板
for imagePath in glob.glob(args['/home/huangfu/桌面/face1'] + "/泉州晋江机场.tif"):
    # 读取测试图片并将其转化为灰度图片
